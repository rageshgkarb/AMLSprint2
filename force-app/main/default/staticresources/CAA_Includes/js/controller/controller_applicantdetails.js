        ...
        $scope.Complete = function () {
            ...
            serviceApplicantDetails.CompleteEvent(data, $scope.SessionId)
                .then(
                    ...
                    function (error) {
                        serviceApplication.LoadHide(false);
                        var errorFromClass = error.message;
                        if(errorFromClass && errorFromClass.includes("Data Validation Error:")){
                            var errorString = errorFromClass.substring(
                                errorFromClass.indexOf("Data Validation Error:") + 22, 
                                errorFromClass.lastIndexOf(":")
                            );
                            $scope.ErrorAlertText = 'Error:'+errorString;
                            $scope.ApplicantErrorAlertText = 'Error:'+errorString;
                        }
