import {
  ...
  import prof from '@salesforce/schema/User.ProfileId';
  ...
  export default class ObservationFormData extends LightningElement {
   ...
   @track pageMessage = '';
    ...
    @wire(getRecord, { recordId: uId, fields: [prof]}) 
    userDetails({nodata, data}) {
        if (data) {
    ...
    getformData() {
        formData({
            ...
            .then((result) => {
                ...
                console.log('Agent:--',result.form.Agent__c,'---Line Manager:',result.form.Line_Manager__c,'---Assessor:',result.form.Quality_Assessor__c,'--Logged in:',this.loggedInUSer,'--Createdagent:',result.form.CreatedById);
                
                console.log('Permission is:' + this.hasPermission);
                console.log('Profile id:'+ this.currentUserProfileId);


                if(!result.form.CreatedById){
                    this.hasPermission = true;
                    console.log('has permission is:' + this.hasPermission);
                }
                if(this.currentUserProfileId=='00eD0000001QprJIAS'){
                    this.hasPermission = true;
                    console.log('Comp Manager profile id:'+ this.currentUserProfileId);
                    console.log('Permission is:' + this.hasPermission);
                }
